...
class FlipkratScrapper:
    ...
    def findElementsFromPageusingClass(self,element_to_be_searched):
        """
        This function finds all element from the page.
        """
        try:
            element=self.driver.find_elements(By.CLASS_NAME,value=element_to_be_searched)
            return element
        except Exception as e:
            raise Exception(f"(findingElementsFromPageUsingClass) - Something went wrong on searching the element.\n" + str(e))

    def findElementfrompageusingCSSSelector(self,element_to_be_searched):
        """
        This function finds all element from the page.
        """
        try:
            element = self.driver.find_elements(By.CSS_SELECTOR,value=element_to_be_searched)
            return element
        except Exception as e:
            raise Exception(f"(findingElementsFromPageUsingClass) - Something went wrong on searching the element.\n" + str(e))
    ...
    def searchproduct(self, searchstring):
        ...
        try:
            locator = self.getLocatorsObject()
            search_searchbox = self.findElementbyXpath(xpath=locator.getinputsearcharea())
            search_searchbox.send_keys(searchstring)
    ...
    def getProductName(self):
        ...
        try:
            ...
            if self.checkVisibilityOfElement(element_to_be_checked=element):
                ...
                product_name = self.findElementbyXpath(xpath=locator.getproductname()).text
            print(product_name)
            return product_name
    ...
    def getPrice(self):
        ...
        try:
            locator = self.getLocatorsObject()
            origional_price = self.findElementbyClass(classpath=locator.getorigionalpricebyclass()).text
            print(origional_price)
    ...
    def getEMIDetails(self):
        ...
        try:
            ...
            if locator.getViewPlanLinkUsingClass() in self.driver.page_source:
                emi_detail = self.findElementbyXpath(xpath=locator.getEMIdetails()).text
                return emi_detail
            else:
                return "NO EMI Plans"
        except Exception as e:
    ...
    def getRatings(self):
        ...
        try:
            locator = self.getLocatorsObject()
            rating = self.findElementfrompageusingCSSSelector(locator.getRatings())
            return rating
        except Exception as e:
    ...
    def getComments(self):
        ...
        try:
            ...
            if comment_object[0] in self.driver.page_source:
                comment = self.findElementsFromPageusingClass(comment_object[0])
            else:
                comment = self.findElementsFromPageusingClass(comment_object[1])
            return comment
        except Exception as e:
    ...
    def getCustomerNamesAndReviewAge(self):
        ...
        try:
            locator = self.getLocatorsObject()
            customer_name = self.findElementsFromPageusingClass(locator.getCustomerName())
